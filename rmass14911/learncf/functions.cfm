...
<p>A function is a series of instructions that are given to a computer to be processed. A function can be written and then just sit there until it is called upon to do it's job.
    We talked a little bit before about functions when we used now(). Now() is a function built in ColdFusion that gets the time from the operating system.
    You can also write your own functions as well. This allows you to break your code into smaller chunks which makes them easier to read and easier to manage.
    It also allows us to write certain functionality that we are going to use over and over again in a way that we only have to write it once.
...
#addNumbers(10,5)#<br>
&lt;/cfoutput&gt;</p>
    <p>We also can assign variables to the value returned from a function. We can write &lt;cfset myNumber=addNumbers(5,8)&gt; will assign the value of 12 to the variable myNumber</p>
...
<p><strong>Bonus Exercise</strong>: Can you make a form that accepts two numbers, submits it to the same page and then uses the function to add the two numbers together? All the information you need is in these docs. </p>
